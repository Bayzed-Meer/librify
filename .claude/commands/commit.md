...
allowed-tools: Bash(git status:*), Bash(git diff:*), Bash(git add:*), Bash(git commit:*), Bash(git log:*), Bash(npm run:*), Bash(ng build:*)
---

# Commit Changes

Verify correctness, stage files, and commit with a Conventional Commit message. Husky will run lint-staged and CommitLint automatically on commit.
...
- Scope: the feature or layer being changed (e.g. `books`, `auth`, `store`, `routing`)
...
Husky will automatically run lint-staged (Prettier + ESLint) and CommitLint. If the hook fails, fix the underlying issue — never use `--no-verify`.
