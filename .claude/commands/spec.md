...
> List the endpoints (method + path) and the key fields in request/response.
> If it's purely frontend state, describe the NgRx slice shape instead."

### Q4 — Edge Cases & Error States
> "What could go wrong, or what unusual inputs should the UI handle gracefully?
> (network errors, empty lists, permission denied, very long strings, concurrent edits, etc.)"
...
> (e.g. p95 search latency < 500 ms, zero JS errors in Sentry for this flow)"
...
<endpoints or NgRx slice shape from Q3>
